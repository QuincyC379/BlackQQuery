...
define('TMP_PATH', PROJECT_PATH . '/system/core/log');
//cp错误类
class cpError extends Exception
{
    ...
    {
        $log_path = cpConfig::get('LOG_PATH');
        $filename = TMP_PATH . '/logger.' . date('Y-m-d') . '.log';
        //@error_log($log_path.PHP_EOL, 3, $filename);
        //检查日志记录目录是否存在
        if (!is_dir($log_path)) {
            //创建日志记录目录
            @mkdir($log_path, 0777, true);
        ...
        //@error_log($destination.PHP_EOL, 3, $filename);
    ...
    protected function logger($msg, $echo = true)
    {
        if (is_array($msg)) {
