...
/**
 * 判断两个日期是否在同一个星期内
 * @param $date1 eg: 2018-06-15
 * @param $date2 eg: 2018-06-20
 * @return bool
 */
function is_week($date1, $date2)
{
    $date1 = date('Y-m-d', strtotime($date1));
    $date2 = date('Y-m-d', strtotime($date2));

    if (strtotime($date1) > strtotime($date2)) {
        $maxTime = strtotime($date1);
        $minTime = strtotime($date2);
    } else {
        $maxTime = strtotime($date2);
        $minTime = strtotime($date1);
    }
