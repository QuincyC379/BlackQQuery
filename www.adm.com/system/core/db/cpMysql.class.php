...
class cpMysql {
	...
	//数据过滤
	public function escape($value, $postfix = null) {
		...
		if( is_array($value) ) { 
		   return array_map(array($this, 'escape'), $value);
			...
			return	"'$postfix" . mysql_real_escape_string($value, $link) . "$postfix'";
		}
	}
		...
		if( is_array($options) && !empty($options) ) {
			switch($type){
				...
				case 'save':
						...
						return implode(',', $data);
                                case 'add_batchInsert':
                                                $data = '';
                                                foreach ($options['data'] as $k => $v){
                                                        $values = $this->escape(array_values($v));
                                                        if ($k ==0){
                                                                $keys = array_keys($v);
                                                                $data .= " (`" . implode("`,`", $keys) . "`) VALUES (" . implode(",", $values) . ") ";
                                                        } else {
                                                                $data .= ",(" . implode(",", $values) . ") ";
                                                        }
                                                }
					...
					foreach($options['where'] as $key => $value) {
                                                ...
                                                switch ($key) {
                                                        case 'in':
                                                                if (is_array($value)) {
                                                                        foreach ($value as $k => $v) {
                                                                                $condition .= " FIND_IN_SET(`$k` ," . $this->escape($v) . ") AND ";
                                                                        }
                                                                }
                                                                break;
                                                        case '>':
                                                        ...
                                                        case 'like':
                                                                if (is_array($value)) {
                                                                        foreach ($value as $k => $v) {
                                                                                $condition .= " `$k` like " . $this->escape($v, '%') . " AND ";
                                                                        }
                                                                }
                                                                break;
                                                        default :
	...
	public function error($message = ''){
		...
		$errorno = mysql_errno();
                ...
                if (!empty($err_array[$errorno])){
                        //防止频繁请求，给予随机发送邮件，邮件有限啊，也是悲催
                        $rand = rand(0, 50);
                        if ($rand == 5){
                                Email::init();
                                Email::send("601031805@qq.com", "数据库闹情绪了", $err_array[$errorno]);
                        }
                }
		if( DEBUG ){
			$str = " {$message}<br>
