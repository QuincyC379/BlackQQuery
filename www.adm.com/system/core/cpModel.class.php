...
class cpModel{
    ...
    public function __call($method, $args) {
        ...
        if ( in_array($method, array('field','data','where','group','having','order','limit','cache','write')) ) {
	...
	private  function _readCache() {
		...
		if ($this->options['cache'] == 0)
			return false;
		if ($this->initCache()) {
			$data = $this->cache->get(md5($this->sql));
			if ( !empty($data) ) {
	...
	private function _writeCache($data) {
		...
		if ( $this->initCache() ) {
			...
			unset($this->options['cache']);
			return $this->cache->set(md5($this->sql), $data, $expire);
		}
		return false;
	}
